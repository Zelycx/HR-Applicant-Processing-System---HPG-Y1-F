namespace Group1_GUI_DB_OOP_Final_Project
{
    namespace Group1_GUI_DB_OOP_Final_Project
    {
        ...
        {
            // ── Color palette ────────────────────────────────────────────────────────
            public static Color Primary = Color.FromArgb(26, 86, 219);  // blue
            public static Color Accent = Color.FromArgb(16, 185, 129);  // green
            ...
            //    Returns the wrapper Panel; 'passwordBox' is the inner TextBox ref.
            public static Panel MakePasswordField(int width, out TextBox passwordBox)
            {
                // Inner textbox (no border – the wrapper panel provides it)
                var tb = new TextBox
                {
                    Font = InputFont,
                ...
                btnEye.Click += (s, e) =>
                {
                    ...
                    btnEye.Text = shown ? "🙈" : "👁";
                    tb.Focus();
                    tb.SelectionStart = tb.Text.Length; // keep cursor at end
                };

                // Wrapper panel acts as the visible border
        ...
        // ════════════════════════════════════════════════════════════════════════════
        //  MOCK DATA  –  replace each method body with your real ADO.NET / EF calls
        // ════════════════════════════════════════════════════════════════════════════
        static class MockDB
        {
            ...
            /// <summary>Replace with: SELECT COUNT(*) FROM Applicants WHERE Email=@e AND Password=@p</summary>
            public static bool Login(string email, string password)
                => email == Email && password == Password && Email != "";

            /// <summary>Replace with: INSERT INTO Applicants (Email,Password,FirstName,LastName) VALUES (@e,@p,@f,@l)</summary>
            public static bool Register(string email, string password,
                                        string firstName, string lastName)
            {
                if (Email != "") return false;   // mock: only one account at a time
                Email = email; Password = password;
                FirstName = firstName; LastName = lastName;
                return true;
            }

            /// <summary>Replace with: UPDATE Applicants SET FirstName=@f, LastName=@l, Phone=@ph, Address=@a WHERE Email=@e</summary>
        ...
        // ════════════════════════════════════════════════════════════════════════════
        //  3. DASHBOARD FORM
        //  FIX: sidebar added LAST so it paints on top; lblWelcome exposed so
        //       ProfileForm can update it after saving.
        // ════════════════════════════════════════════════════════════════════════════
        class DashboardForm : Form
        {
            // Exposed so ProfileForm can refresh it
            ...
            void BuildUI()
            {
                // ── Main content panel (added FIRST = behind sidebar) ────────────────
                var main = new Panel
                {
                ...
                // ── Sidebar (added LAST = rendered on top of main) ───────────────────
                ...
                // Welcome label – stored in field so ProfileForm can update it
        ...
        // ════════════════════════════════════════════════════════════════════════════
        //  4. PROFILE FORM
        //  FIX: receives DashboardForm reference so it can update the sidebar label
        ...
        class ProfileForm : Form
        {
            ...
            // Reference to dashboard – used to refresh the welcome label
            ...
            void BuildUI()
            {
                ...
                // Pre-fill from MockDB  (replace with SQL SELECT by email)
            ...
            void BtnSave_Click(object? s, EventArgs e)
            {
                ...
                // ── Update sidebar welcome label live ────────────────────────────────
                if (_dashboard != null)
